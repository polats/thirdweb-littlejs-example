...
// replace with your own client id and secret key from https://thirdweb.com/dashboard/settings/api-keys
const THIRDWEB_SECRET_KEY="";
const ARCADIANS_ETH_ADDRESS="0xc3c8a1e1ce5386258176400541922c414e1b35fd";
...
async function loadThirdWeb() {
    ...
    const contract = ThirdWeb.getContract(
        {
            client,
            address: ARCADIANS_ETH_ADDRESS,
            chain: ethereum   
        }
    )

    const nftData = await erc721.getNFT(
        {
            contract,
            tokenId: 1n
        }
...
function gameInit()
{
    ...
    let tileImage;
    let texture: LittleJS.TextureInfo = LittleJS.textureInfos[0];
    ...
    particleEmitter = new LittleJS.ParticleEmitter(
        ...
        tile((vec2(0,0)), vec2(0,0), 16),                // tileIndex, tileSize
...
function gameRender()
{
    ...
    // LittleJS.drawTile(vec2(21,5), vec2(4.5), tile((vec2(0,0)), vec2(0,0),128));
}
...
function gameRenderPost()
{
    ...
    LittleJS.drawTextScreen('LittleJS + Third Web', vec2(LittleJS.mainCanvasSize.x/2, 80), 80);
    LittleJS.drawTextScreen(tokenURI, vec2(LittleJS.mainCanvasSize.x/2, 120), 20);
